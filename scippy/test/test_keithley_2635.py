...
"""
from scippy import Keithley2635 as Keithley
from scippy import ureg
...
def keithley():
    print("Opening Keithley 2635 device...")
    keith = Keithley()
    keithley_parameters = {
        'device': keith,
        'id': 'Keithley Instruments Inc., Model 2635, 1212537, 1.4.1',
    }
...
@pytest.mark.keithley2635
def test_set_voltage_level(keithley, timeout):
...
def test_measure_voltage(keithley, timeout):
    ...
    assert abs(actual_voltage - desired_voltage) < 1*ureg.mV
    assert abs(actual_current) < 100*ureg.fA
